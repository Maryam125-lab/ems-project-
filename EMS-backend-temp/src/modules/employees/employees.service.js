...
export async function createEmployee(data, createdByUserId) {
  ...
  const { personalInfo, jobInfo, salaryInfo, accountInfo, emergencyContacts, bankInfo, medicalInfo, documentInfo, accessInfo } = data;
  ...
  try {
    ...
    const portalAccessEnabled = accessInfo?.portal_access !== 'disabled';
    ...
    if (!portalAccessEnabled) {
      await client.query(
        `UPDATE public.users SET is_active = false WHERE employee_id = $1`,
        [employeeId]
      );
    }
    ...
    // Insert salary details if provided during onboarding.
    if (salaryInfo && (
      salaryInfo.base_salary != null ||
      salaryInfo.payroll_cycle ||
      salaryInfo.tax_status ||
      salaryInfo.allowance_notes ||
      salaryInfo.probation_salary != null ||
      salaryInfo.salary_effective_from
    )) {
      await client.query(
        `
          ...
          VALUES (gen_random_uuid(), $1, $2, 'PKR', COALESCE($3::date, CURRENT_DATE), true, true, 'Initial', $4, $5, $6, $7, $8)
        `,
        [
          employeeId,
          salaryInfo.base_salary || 0,
          salaryInfo.salary_effective_from || null,
          salaryInfo.payroll_cycle || null,
          salaryInfo.tax_status || null,
          salaryInfo.allowance_notes || null,
          salaryInfo.probation_salary || null,
          createdByUserId,
        ]
      );
    ...
    if (documentInfo) {
      const documents = [
        ['cnic', documentInfo.cnic_document],
        ['resume', documentInfo.resume_document],
        ['offer_letter', documentInfo.offer_letter_document],
      ].filter(([, fileName]) => fileName);

      for (const [documentType, fileName] of documents) {
        await client.query(
          `
          ...
          `,
          [employeeId, documentType, fileName, createdByUserId]
        );
      }
    }
    ...
    if (accessInfo) {
      await client.query(
        `
          INSERT INTO public.employee_access_settings (
            employee_id,
            portal_access,
            access_role,
            send_welcome_email,
        ...
        [
          employeeId,
          portalAccessEnabled,
          accessInfo.access_role || 'employee',
          accessInfo.send_welcome_email !== 'no',
          createdByUserId,
        ]
      );
    }
    ...
    // Log employee onboarding in the background
    logAudit({
      userId: createdByUserId,
      action: 'INSERT',
      tableName: 'employee_info',
      recordId: empUUID,
      reason: `Onboarded new employee ${personalInfo.name} (${employeeId})`
    });
...
export async function getEmployeeById(employeeId) {
  const result = await pool.query(
    `
      SELECT
        ...
        -- Onboarding access and documents
        eas.portal_access,
        eas.access_role,
        eas.send_welcome_email,
  ...
  const fieldsToRemove = [
    ...
    'payroll_cycle', 'tax_status', 'allowance_notes', 'probation_salary', 'salary_effective_from',
    'portal_access', 'access_role', 'send_welcome_email'
  ];
  
  fieldsToRemove.forEach(field => delete employee[field]);

  // Explicitly populate flat properties for frontend Details.cshtml compatibility
  ...
  employee.marital_status = row.marital_status || 'single';
